...
AISUMMONER_DEFAULT_SERVER_ORIGIN ?= https://122.51.70.33:10001
...
remote-ui:
	cmake -S desktop/remote-client -B "$(REMOTE_UI_BUILD)" -DCMAKE_BUILD_TYPE=RelWithDebInfo -DBUILD_TESTING=ON -DAISUMMONER_DEFAULT_SERVER_ORIGIN="$(AISUMMONER_DEFAULT_SERVER_ORIGIN)"
...
remote-appimage:
	AISUMMONER_DEFAULT_SERVER_ORIGIN="$(AISUMMONER_DEFAULT_SERVER_ORIGIN)" ./deploy/build-remote-client-appimage.sh "$(REMOTE_APPIMAGE_OUTPUT)"
